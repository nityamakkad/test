...
def _parse_date(series: pd.Series) -> pd.Series:
    # Try MM/DD/YYYY first (US format used in this data), fallback to auto
    parsed = pd.to_datetime(series, errors="coerce", format="%m/%d/%Y")
    # For any that failed, try dayfirst=True (e.g. 27-Mar-2026)
...
def load_refund(path: str = "refund_data.csv") -> pd.DataFrame:
    ...
    #   days_enroll_to_cohort_start < 0 means student enrolled after cohort started (joined mid-cohort)
    #   days_cohort_to_refund_req < 0 means student refunded before their future cohort started (early enrollee)
