...
def chart_quarterly_enroll_vs_refund(df_r: pd.DataFrame, df_o: pd.DataFrame) -> go.Figure:
    """
    Combo chart like manager dashboard:
    Blue bars = enrollments per quarter (enrollment cohort)
    Red line+dots = refund count for that quarter (refund processed date)
    Dual Y-axis so both scales are readable.
    """
    enr = df_o.groupby("enrollment_quarter").size().rename("enrollments").reset_index()
    ref = df_r.groupby("refund_quarter").size().rename("refunds").reset_index()
    ref.columns = ["enrollment_quarter","refunds"]
    ...
    fig.update_yaxes(title_text="Refund Count", secondary_y=True,
                     showgrid=False)
    fig.update_xaxes(showgrid=False)
    fig.update_layout(
        title="Quarterly Enrollments & Refund Count",
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
...
def chart_monthly_refund_line(df_r: pd.DataFrame, df_o: pd.DataFrame) -> go.Figure:
    """
    Monthly view: bars for enrollments + red line for refunds.
    Same combo style as quarterly chart above for visual consistency.
    Uses enrollment month for both to align cohorts.
    """
    ...
    merged["refunds"] = merged["refunds"].astype(int)
    merged = merged[merged["enrollment_month"].isin(month_order)].sort_values("enrollment_month")
    merged["label"] = merged["enrollment_month"].map(month_labels)
    ...
    fig.add_trace(go.Scatter(
        ...
        name="Refund Count (by enroll month)", mode="lines+markers+text",
        text=merged["refunds"],
        textposition="top center",
        textfont=dict(color=C_RED, size=12, family="Inter, sans-serif"),
        line=dict(color=C_RED, width=2.5),
    ...
    fig.update_xaxes(showgrid=False)
    fig.update_layout(
        title="Monthly Enrollments & Refund Count (by Enrollment Cohort)",
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        **{k: v for k, v in CHART_LAYOUT.items() if k != "legend"})
    return fig
...
def chart_new_vs_alumni(df: pd.DataFrame) -> go.Figure:
    ...
    grp.columns = ["type","count"]
    color_map = {"New":C_BLUE,"Alumni":C_AMBER,"Unknown":C_SLATE,"Not Updated":C_SLATE}
