...
st.markdown('<div class="section-header">① Quarterly & Monthly Trends</div>',
            unsafe_allow_html=True)
# Chart 1: Quarterly combo (bars = enrollments, red line = refunds)
st.plotly_chart(chart_quarterly_enroll_vs_refund(df, df_onboard_filtered),
                use_container_width=True)
# Chart 2: Monthly combo — same style, one below the other
...
st.markdown(
    ...
    'refund rate for this month will rise. Use Apr–May 2026 as the most mature comparison window.</div>',
    unsafe_allow_html=True)
...
st.markdown('<div class="section-header">② Segmentation — BU, Category, Course</div>',
            unsafe_allow_html=True)
# BU pies — smaller columns so donuts don't stretch too wide
_, pie_l, pie_r, _ = st.columns([1, 2, 2, 1])
with pie_l:
...
# Category and Course each get full width on their own row
